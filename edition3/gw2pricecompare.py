...
def fetch_30_entry_average(item_id):
    try:
        ...
        for entry in history_response:
            entry["date"] = datetime.strptime(entry["date"], "%Y-%m-%dT%H:%M:%S.%fZ").replace(tzinfo=timezone.utc)

        # Sort entries by date and get the last 30 entries
        history_response = sorted(history_response, key=lambda x: x["date"], reverse=True)[:30]

        # Extract the minimum sell prices
        prices = [entry["sell_price_min"] for entry in history_response if "sell_price_min" in entry]
